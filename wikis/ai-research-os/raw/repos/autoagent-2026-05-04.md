...
## Summary
AutoAgent is a framework for autonomous agent-harness engineering: a human programs `program.md`, while a meta-agent edits `agent.py`, runs Harbor benchmark tasks, checks scores, and iterates. It treats the harness as the editable target rather than the model itself.
...
- **Watchers:** 29
- **Open issues:** 7
- **Created:** 2026-04-02T20:44:58Z
- **Updated:** 2026-05-04T17:41:34Z
- **Last pushed:** 2026-04-03T21:11:51Z
- **Default branch:** main
- **Homepage:** none
- **Topics:** none listed
...
## Key points
- Uses a single-file harness (`agent.py`) with editable prompt/tool/routing sections plus fixed Harbor adapter boundaries.
- Human control is concentrated in `program.md`, which defines the agent-engineering loop and target behavior.
- Runs Harbor-format benchmark tasks and hill-climbs on total score.

## Why it matters
Useful companion to auto-harness/HALO for Self-OS: it formalizes “program the loop, let the meta-agent improve the harness” as a lightweight repo pattern.
